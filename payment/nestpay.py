...
class NestPay:
    ...
    def bank_data(self, *args, **kwargs):
        ...
        string_to_hash = utils.convert_string_to_hash(string).decode()
    ...
    def data_result(self, **kwargs):
        ...
        while index1 < len(hashparams):
            ...
            s = utils.substr(hashparams, index1, index2 - index1)
            vl = kwargs.get(f"{s}")
            paramsval = paramsval + vl
            index1 = index2 + 1
        hashval = paramsval + storekey
        hashcon = utils.convert_string_to_hash(hashval).decode()
        if hashparams != None:
            ...
            return (
                "Hash values error. Please check parameters posted to 3D secure page."
            )
